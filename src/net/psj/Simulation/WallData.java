...
public class WallData {
	...
	public static Wall[][] bmap = new Wall[PowderSimJ.height / PowderSimJ.cell][PowderSimJ.width
			/ PowderSimJ.cell];

	public Wall create_wall(int x, int y, int id) {
		if (x > PowderSimJ.width / PowderSimJ.cell - 1 || x < 1
				|| y > PowderSimJ.height / PowderSimJ.cell - 1 || y < 1) {
			return null;
		}
		Wall newWall = newWallfromID(id);
		if (!(newWall instanceof WallErase) && bmap[y][x] == null
				&& newWall != null) {
			bmap[y][x] = newWall;
	...
	public static Wall newWallfromID(int id) {
		...
		if (id == 0)
			return new WallErase();
		if (id == 1)
			return new WallBasic();
		if (id == 2)
			return new WallFan();
		return null;
	}
