...
public class PowderSimJ extends BasicGame implements MouseListener, KeyListener {
	...
	while (hasGotNatives == false) {
	    if(hasGotNatives) break;
	}
	System.setProperty("org.lwjgl.librarypath", getDirectory()
		+ "/natives/" + getOs() + "");
	...
	if (isSettingFan)
	    RenderUtils.drawLine(fanX, fanY, mouseX, mouseY, 1, 1.0f, 1.0f,
		    1.0f);
	else {
	    int x1 = mouseX, y1 = mouseY;
	    x1 = x1 - (PowderSimJ.brushSize / 2);
	    y1 = y1 - (PowderSimJ.brushSize / 2);
	    RenderUtils.drawRectLine(x1, y1, x1 + brushSize, y1 + brushSize,
		    1.0f, 1.0f, 1.0f);
	}
	...
	if (mouseY > 0 && mouseY < height) {
		...
		while (!(mouseX % cell == 0))
		    mouseX--;
		if (WallData.bmap[mouseY / cell][mouseX / cell] instanceof WallFan
