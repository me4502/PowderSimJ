	...
	public boolean update() {
		...
		for (int i = -3; i < 2; i++)
			for (int p = -3; p < 2; p++) {
				...
				if((part.explosive>0 || part.flammable > 0) && (part.temp < 2273.15f && Air.pv[(int)y/CELL][(int)x/CELL] < 50.0f) && part.flammable > 0 && (part.flammable + (int)(Air.pv[(int)(y+i)/CELL][(int)(x+p)/CELL]*10.0f))>(rand.nextInt(1000)))
				{
