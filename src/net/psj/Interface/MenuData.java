...
public class MenuData {
	...
	{
		if(id == 1) return MENU_WALLS;
		if(id == 0) return MENU_PARTS;
		else return null;
	}
	...
	static void draw_menu(int i)
	{
		...
		if (selected==i)//selected menu
		{
			...
			RenderUtils.drawFont((PowderSimJ.width+PowderSimJ.barSize)-13, (i*16)+PowderSimJ.height+PowderSimJ.menuSize-14-(menus*16), getMenuFromID(i).name, 0, 0, 0);
		}
		else //unselected menu
		{
			RenderUtils.drawFont((PowderSimJ.width+PowderSimJ.barSize)-13, (i*16)+PowderSimJ.height+PowderSimJ.menuSize-14-(menus*16), getMenuFromID(i).name, 255, 255, 255);
		}
	}
}
