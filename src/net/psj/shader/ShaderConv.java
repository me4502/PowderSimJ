...
public class ShaderConv {
	...
	public ShaderConv(String VertexFilename, String FragmentFilename,
			String imgPath) {
		try {
			// Tengo via la mia immagine
			img = new Image(imgPath);
		} catch (SlickException ex) {
			Logger.getLogger(ShaderConv.class.getName()).log(Level.SEVERE,
					null, ex);
		}
		...
		ARBShaderObjects.glShaderSourceARB(fragmentShaderId,
				getProgramCode(FragmentFilename));
		...
		widthPos = ARBShaderObjects.glGetUniformLocationARB(shaderProgramId,
				toByteString("width"));
		heightPos = ARBShaderObjects.glGetUniformLocationARB(shaderProgramId,
				toByteString("height"));
